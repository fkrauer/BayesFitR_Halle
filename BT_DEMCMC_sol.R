...
fit1 <- trajsim_Pois(chain1, theta, inits, times, model, ndraw=999, nburn=nburn)
# the rest of the commands for plotting the fit is identical to custom1
...
lower2 = c("beta"=1e-6, "sigma"=1e-6, "gamma"=1e-6, "rho"=1e-6)
upper2 = c("beta"=3.0, "sigma"=1.0, "gamma"=1.0, "rho"=1.0)
...
ll_Pois2 <- function(theta, inits, times, data) {
...
# test
ll_Pois2(theta2, inits, times, data2)
...
trajsim_Pois2 <- function(chain, theta, inits, times, model, ndraw, nburn) {
...
fit2 <- trajsim_Pois2(chain2, theta2, inits, times, model, ndraw=999, nburn=nburn)
fit2$replicate <- as.numeric(fit2$replicate)

# Calculate quantiles of the trajectory
fit2_quantiles <- plyr::ddply(.data=fit2, 
                             .variables="time", 
                             function(x) quantile(x[, which(colnames(fit2)=="simobs")], 
                                                  prob = c(0.025, 0.25, 0.5, 0.75, 0.975), na.rm=T)) 
colnames(fit2_quantiles) <- c("time", "low95", "low50", "median", "up50", "up95")

fit2_quantiles <- merge(fit2_quantiles, data2[,c("time", "obs")], by="time", all=T)

# Plot fit
ggplot(fit2_quantiles) +
  geom_point(aes(x=time, y=obs)) +
  geom_line(aes(x=time, y=median), color="red") +
  geom_line(data=traj, aes(x=time, y=inc), color="black") +
...
lower3 = c("beta"=1e-6, "sigma"=1e-6, "gamma"=1e-6, "rho"=1e-6, "k"=0.0)
upper3 = c("beta"=3.0, "sigma"=1.0, "gamma"=1.0, "rho"=1.0, "k"=1.0)
...
ll_NB3 <- function(theta, inits, times, data) {
  
  traj <- model(times, inits, theta)
  datapoint <- data$obs
  modelpoint <- traj$inc * theta[["rho"]]
...
# test
ll_NB3(theta3, inits, times, data3)
...
trajsim_NB <- function(chain, theta, inits, times, model, ndraw, nburn) {
...
fit3 <- trajsim_NB(chain3, theta3, inits, times, model, ndraw=999, nburn=nburn)
fit3$replicate <- as.numeric(fit3$replicate)

# Calculate quantiles of the trajectory
fit3_quantiles <- plyr::ddply(.data=fit3, 
                             .variables="time", 
                             function(x) quantile(x[, which(colnames(fit3)=="simobs")], prob = c(0.025, 0.25, 0.5, 0.75, 0.975), na.rm=T)) 
colnames(fit3_quantiles) <- c("time", "low95", "low50", "median", "up50", "up95")

fit3_quantiles <- merge(fit3_quantiles, data3[,c("time", "obs")], by="time", all=T)

# Plot fit
ggplot(fit3_quantiles) +
  geom_point(aes(x=time, y=obs)) +
  geom_line(aes(x=time, y=median), color="red") +
  geom_ribbon(aes(x=time, ymin=low95, ymax=up95), alpha=0.2, fill="red")
