...
Note that I have suppressed the messages to the console for this tutorial with `message = FALSE`, but you should have message enabled (the default) to monitor the progress of the fitting. If message is enabled, you will see the current iteration as well as the current log posterior. If you see that the log posterior value does not change at all, the chain is not moving and there may be something wrong with the model or the fitting. 
...
The DEzs sampler is more performant if your model is complex and there is some parameter correlation. The algorithm runs one group of 3 chains as a default. So the argument `nrChains` in the MCMC settings denotes the number of groups of 3. These three chains are not independent of each other, so to assess convergence, I recommend to run at least `nrChains=2` (which are 2*3 chains). The syntax is very similar to the Metropolis sampler:
